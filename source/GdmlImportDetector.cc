...
GdmlImportDetector::GdmlImportDetector(PHG4Subsystem *subsys,
  ...
  , m_DisplayAction(dynamic_cast<GdmlImportDetectorDisplayAction *>(subsys->GetDisplayAction()))
  , m_Params(parameters)
...
int GdmlImportDetector::IsInDetector(G4VPhysicalVolume *volume) const
{
  if (m_Active)
  {
...
//_______________________________________________________________
void GdmlImportDetector::ConstructMe(G4LogicalVolume *logicWorld)
{
  ...
  GdmlImportDetectorSubsystem *mysubsys = dynamic_cast<GdmlImportDetectorSubsystem *>(GetMySubsystem());
...
void GdmlImportDetector::InsertVolumes(G4VPhysicalVolume *physvol, const int flag)
{
  ...
  if (flag == insertassemblies)
  {
    ...
    if (physvol->GetName().find("av_") != string::npos && physvol->GetName().find("_impr_") != string::npos)
    {
      ...
      {
		//Loop through the different prefixes, and set detector indices. Need to add all the prefixes here.
		//10: SVT
		//20: Time-stamping layer(s)
		//30: Silicon TPC
		//40: Forward disks
		//50: Backward disks
        string detprefix[] = {"VstStave", "TimingStave", "SitpcStave", "FstContainerVolume", "BstContainerVolume"};
    ...
    if (detid < 0)
    {
      cout << "GdmlImportDetector::InsertVolumes detid is " << detid << " vol name " << physvol->GetName() << endl;
      gSystem->Exit(1);
    }
  }
  else
  ...
  if (physvol->GetName().find("MimosaCore") != string::npos)
  {
    ...
    //std::cout << "Detector volume here!" << std::endl;
  }
  else
  {
    if (m_AbsorberActive)
    {
      ...
      //std::cout << "Absorber volume here!" << std::endl;
    }
  }
  ...
  for (int i = 0; i < (int) logvol->GetNoDaughters(); ++i)
  {
    ...
    InsertVolumes(physvol, flag); //Recursive call
  }
  return;
}

//_______________________________________________________________
void GdmlImportDetector::Print(const std::string &what) const
{
  cout << "GdmlImportDetector Detector:" << endl;
...
int GdmlImportDetector::get_detid(const G4VPhysicalVolume *physvol, const int whichactive)
{
  if (whichactive > 0)
  {
  ...
  {
    cout << "GdmlImportDetector::get_detid invalid whichactive flag = 0" << endl;
    gSystem->Exit(1);
  }
  return 0;
}

void GdmlImportDetector::AddHitNodes(PHCompositeNode *topNode)
{
  ...
  if (m_Params->get_int_param("active"))
  {
    ...
    for (auto iter = DetidSet.begin(); iter != DetidSet.end(); ++iter)
    {
      ...
      g4hitnodeactive << "G4HIT_" << myname << "_" << *iter;
...
PHG4HitContainer * GdmlImportDetector::get_hitcontainer(const int i)
{
